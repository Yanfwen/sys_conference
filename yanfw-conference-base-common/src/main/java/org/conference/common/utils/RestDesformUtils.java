...
public class RestDesformUtils {
    ...
    public static Result queryOne(String desformCode, String dataId, String token) {
        ...
        ResponseEntity<JSONObject> result = RestUtils.request(url, HttpMethod.GET, headers, null, null, JSONObject.class);
    ...
    private static Result addOrEditOne(String desformCode, JSONObject formData, String token, HttpMethod method) {
        ...
        HttpHeaders headers = getHeaders(token);
        ResponseEntity<JSONObject> result = RestUtils.request(url, method, headers, null, formData, JSONObject.class);
        return packageReturn(result);
    }
    ...
    public static Result removeOne(String desformCode, String dataId, String token) {
        String url = getBaseUrl(desformCode, dataId).toString();
        HttpHeaders headers = getHeaders(token);
        ResponseEntity<JSONObject> result = RestUtils.request(url, HttpMethod.DELETE, headers, null, null, JSONObject.class);
        return packageReturn(result);
    }
